...
class WanVideoSampler:
    ...
    def process(self, model, image_embeds, shift, steps, cfg, seed, scheduler, riflex_freq_index, text_embeds=None,
        ...
        prompt_embeddings = text_embeds.get("prompt_embeds", [])
        prompt_texts = text_embeds.get("prompt_texts", [])
        prompt_switch_gap = text_embeds.get("textencode_gap", 1)
        try:
            prompt_switch_gap = int(prompt_switch_gap)
        except (TypeError, ValueError):
            prompt_switch_gap = 1
        if text_embeds.get("echoshot", False):
            prompt_switch_gap = 1
        if prompt_switch_gap < 1:
            prompt_switch_gap = 1

        reset_condition_on_prompt_change = text_embeds.get("reset_condition_on_prompt_change", False)

        last_prompt_index = None
        current_iteration_prompt_index = None

        def reset_prompt_cache_states():
            if hasattr(self, "cache_state") and isinstance(self.cache_state, list):
                self.cache_state = [None] * len(self.cache_state)
            if hasattr(self, "cache_state_source") and isinstance(self.cache_state_source, list):
                self.cache_state_source = [None] * len(self.cache_state_source)
            if hasattr(self, "cache_states_context"):
            ...
            if hasattr(transformer, "teacache_state") and hasattr(transformer.teacache_state, "clear_all"):
                try:
                    ...
                    pass
            if hasattr(transformer, "magcache_state") and hasattr(transformer.magcache_state, "clear_all"):
                try:
                    transformer.magcache_state.clear_all()
                except Exception:
                    pass
            if hasattr(transformer, "easycache_state") and hasattr(transformer.easycache_state, "clear_all"):
                try:
                    transformer.easycache_state.clear_all()
                except Exception:
                    pass

        def get_positive_prompt(iteration_idx, wrap=True):
            nonlocal last_prompt_index
            prompt_changed = False
            if len(prompt_embeddings) > 1:
                try:
                    iteration_value = int(iteration_idx)
                except (TypeError, ValueError):
                    iteration_value = 0
                if iteration_value < 0:
                    iteration_value = 0
                raw_index = iteration_value // prompt_switch_gap
                prompt_idx = raw_index % len(prompt_embeddings)
                if last_prompt_index is None or prompt_idx != last_prompt_index:
                    reset_prompt_cache_states()
                    prompt_changed = True
                    last_prompt_index = prompt_idx
                else:
                    prompt_changed = False
                selected = prompt_embeddings[prompt_idx]
                if torch.is_tensor(selected):
                    selected = selected.clone()
                elif isinstance(selected, (list, tuple)):
                    selected = [tensor.clone() if torch.is_tensor(tensor) else tensor for tensor in selected]
                return ([selected] if wrap else selected, prompt_idx, prompt_changed)
            return (prompt_embeddings, 0, False)
        
        def prompt_preview_text(prompt_idx):
            if not prompt_texts or not isinstance(prompt_texts, list):
                return ""
            if 0 <= prompt_idx < len(prompt_texts):
                text = prompt_texts[prompt_idx]
                if not isinstance(text, str):
                    return ""
                return text if len(text) <= 80 else text[:80] + "…"
            return ""

        def prompt_embed_checksum(embed_value):
            tensor = None
            if isinstance(embed_value, list) and embed_value:
                tensor = embed_value[0] if torch.is_tensor(embed_value[0]) else None
            elif torch.is_tensor(embed_value):
                tensor = embed_value
        ...
        def clone_positive_embed(positive_value):
            if isinstance(positive_value, list):
                return [tensor.clone() if torch.is_tensor(tensor) else tensor for tensor in positive_value]
            if torch.is_tensor(positive_value):
                return positive_value.clone()
                ...
                for idx, t in enumerate(tqdm(timesteps, disable=multitalk_sampling or wananimate_loop)):
                    ...
                    if flowedit_args is not None:
                        ...
                        if context_options is not None:
                            ...
                            for c in context_queue:
                                ...
                                positive, prompt_index, _ = get_positive_prompt(window_id, wrap=False)
                                if context_options["verbose"] and len(prompt_embeddings) > 1:
                                    preview = prompt_preview_text(prompt_index)
                                    checksum = prompt_embed_checksum(positive)
                                    log_line = f"Prompt index: {prompt_index}"
                                    if preview:
                                        log_line += f" -> {preview}"
                                    if checksum is not None:
                                        log_line += f" (mean {checksum:.6f})"
                                    log.info(log_line)
                            ...
                            positive, prompt_index, _ = get_positive_prompt(idx)
                    ...
                    #region context windowing
                        ...
                        for i, c in enumerate(context_queue):
                            ...
                            positive, prompt_index, _ = get_positive_prompt(window_id)
                            if context_options["verbose"] and len(prompt_embeddings) > 1:
                                preview = prompt_preview_text(prompt_index)
                                checksum = prompt_embed_checksum(positive)
                                log_line = f"Prompt index: {prompt_index}"
                    ...
                    #region multitalk
                        ...
                        while True: # start video generation iteratively
                            ...
                            current_positive_wrapped, current_prompt_index, _ = get_positive_prompt(iteration_count)
                            if isinstance(current_positive_wrapped, list) and current_positive_wrapped:
                                current_positive_unwrapped = current_positive_wrapped[0]
                            else:
                                current_positive_unwrapped = current_positive_wrapped

                            iteration_prompt_changed = current_iteration_prompt_index is None or current_prompt_index != current_iteration_prompt_index
                            reset_condition_this_iter = iteration_prompt_changed and iteration_count > 0 and reset_condition_on_prompt_change
                            if reset_condition_this_iter:
                                cond_image = None
                                cond_frame = None
                                current_condframe_index = 0
                                is_first_clip = True
                            else:
                                reset_condition_this_iter = False
                            current_iteration_prompt_index = current_prompt_index
                            ...
                            if window_cond_image is None:
                                if reset_condition_this_iter:
                                    if original_images is not None:
                                        window_cond_image = torch.zeros_like(original_images[:, :, 0:1])
                                    elif target_h is not None and target_w is not None:
                                        dtype_to_use = vae.dtype if vae is not None else torch.float32
                                        window_cond_image = torch.zeros(1, 3, 1, target_h, target_w, device=device, dtype=dtype_to_use)
                                if window_cond_image is None:
                            ...
                            positive = clone_positive_embed(current_positive_wrapped)
                            prompt_index = current_prompt_index
                            if len(prompt_embeddings) > 1:
                                preview = prompt_preview_text(prompt_index)
                                checksum = prompt_embed_checksum(current_positive_unwrapped)
                                log_line = f"Using prompt index: {prompt_index}"
                                if preview:
                                    log_line += f" -> {preview}"
                                if checksum is not None:
                                    log_line += f" (mean {checksum:.6f})"
                    ...
                    # region framepack loop
                        ...
                        for r in range(s2v_num_repeat):
                            ...
                            positive, prompt_index, _ = get_positive_prompt(r)
                            if len(prompt_embeddings) > 1:
                                preview = prompt_preview_text(prompt_index)
                                checksum = prompt_embed_checksum(positive)
                                log_line = f"Using prompt index: {prompt_index}"
                    ...
                    #region normal inference
                        ...
                        positive_for_step = clone_positive_embed(current_positive_wrapped)
                        prompt_index = current_prompt_index
                        if len(prompt_embeddings) > 1:
                            preview = prompt_preview_text(prompt_index)
                            checksum = prompt_embed_checksum(current_positive_unwrapped)
                        ...
                        noise_pred, noise_pred_ovi, self.cache_state = predict_with_cfg(
                            latent_model_input, 
                            cfg[idx], positive_for_step, text_embeds["negative_prompt_embeds"],
                            timestep, idx, image_cond, clip_fea, control_latents, vace_data, unianim_data, audio_proj, control_camera_latents, add_cond,
...
NODE_CLASS_MAPPINGS = {
    "WanVideoSamplerMprompt": WanVideoSampler,
    }
NODE_DISPLAY_NAME_MAPPINGS = {
    "WanVideoSamplerMprompt": "WanVideo Sampler Mprompt",
}
